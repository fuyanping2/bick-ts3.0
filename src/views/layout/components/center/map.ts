...
class MyMap {
 ...
 public createWorkPoint(data: any, icon: string): object {
  ...
  // 创建清扫车的路线
  public sweeperLine(row:any): void {
    let polygons:any = []
    row.district.forEach((itam:any)=>{
      polygons.push(new AMap.LngLat(itam[0],itam[1]))
    })
    const polygon: object = new AMap.Polyline({
      path: polygons,
      map: this.map,
      strokeColor: '#78c16e', // 线颜色
    ...
    AMap.event.addListener(polygon, 'click', (e: any) => {
  ...
  //隐藏清扫车
  public clearCar(){
    let allOverlay:any = this.map.getAllOverlays('polyline')
    if (allOverlay.length > 0) {
      allOverlay.forEach((element:any) => {
  ...
  public createInfoWindow (res:string, lnglat:any):void {
    console.log(this.infoWindow)
    // this.infoWindow.setContent(res)
    // this.infoWindow.open(this.map, new AMap.LngLat(lnglat.lng, lnglat.lat))
  }
  // 创建区级点
  ...
  public cleargroup(): void{
    if(this.workOrderGroup){
      this.workOrderGroup.clearOverlays()
    }
  }
